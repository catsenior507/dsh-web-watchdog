...
function ensureSupervisor(config) {
	...
	const wmiCmd = "(Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{CommandLine='" + ["powershell.exe", ...args].map((arg) => "\"" + arg.replace(/"/g, "\"\"") + "\"").join(" ").replace(/'/g, "''") + "'}).ReturnValue";
	const caller = spawn("powershell.exe", [
		"-NoProfile",
		"-ExecutionPolicy",
		"Bypass",
		"-Command",
